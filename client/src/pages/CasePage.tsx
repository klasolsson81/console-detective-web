...
import {
  ArrowLeft, Search, Users, Lightbulb, MessageSquare, CheckCircle, Loader
} from 'lucide-react';
    ...
    <div className="min-h-screen bg-noir-darkest pb-12">
      ...
      <div className="container mx-auto px-4 mt-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          {/* Vänsterkolumn: Detaljer & Ledtrådar */}
          <div className="lg:col-span-2 space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="card-noir p-6">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="card-noir p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <button onClick={handleInvestigate} disabled={investigating} className="btn-primary flex items-center gap-2">
                  <Search size={20} />
              ...
              {caseData.clues && caseData.clues.length > 0 ? (
                <div className="space-y-3">
                  {caseData.clues.map((clue, index) => (
                    <motion.div key={clue.id} initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.1 }} className="bg-noir-dark border border-gray-700 p-4 rounded">
          ...
          {/* Högerkolumn: Misstänkta & Lösning */}
          <div className="space-y-6">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="card-noir p-6">
              ...
              <div className="space-y-3">
                {caseData.possibleSuspects.map((suspect) => (
                  <button key={suspect} onClick={() => handleInterrogate(suspect)} className="w-full bg-noir-dark hover:bg-noir-medium border border-gray-700 hover:border-noir-accent p-3 rounded transition-all group">
                    <div className="flex items-center gap-3">
                      <div className="w-12 h-12 rounded-full overflow-hidden border-2 border-gray-600 group-hover:border-noir-accent flex-shrink-0">
                        <img src={getImagePath('suspects', suspect)} alt={suspect} className="w-full h-full object-cover" onError={(e) => { e.currentTarget.style.display = 'none'; }} />
                      </div>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="card-noir p-6 border-2 border-noir-accent/50">
              <h2 className="text-2xl font-noir text-noir-accent mb-4">{t('case.solve')}</h2>
              <p className="text-gray-400 text-sm mb-4">Är du redo att avslöja sanningen?</p>
              <button onClick={() => setShowSolveModal(true)} className="btn-primary w-full">
                {t('case.solve')} →
              </button>
            </motion.div>
          </div>
              ...
              {caseData.possibleSuspects.map((suspect) => (
                <button
                  ...
                  className={`w-full p-4 border-2 rounded transition-all ${
                    selectedSuspect === suspect ? 'border-noir-accent bg-noir-accent/10' : 'border-gray-700 hover:border-gray-600'
                  }`}
                >
                  <div className="flex items-center gap-3">
